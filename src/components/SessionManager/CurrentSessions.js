...
const CurrentSessions = ({ sessions, setIsEditting, reset, setDeletedSession }) => {
  return (
            ...
            <div>
              ...
              <div className="container flex flex-col items-center justify-center mx-auto sm:py-2">
                <div className="flex flex-row flex-wrap justify-center mt-4">
                  { sessions.map(s => <CurrentSessionCard data={s} setIsEditting={setIsEditting} reset={reset} setDeletedSession={setDeletedSession} key={s.id}/>) }
                </div>
              </div>
            </div>
          )}
